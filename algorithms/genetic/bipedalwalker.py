...
bipedalWalker = BipedalWalker(1)
input_layer = 24
hidden_layer = 16
output_layer = 4

class BipedalWalkerIndividual:
    """
...
def run_individual_model(game_model, num_episodes=EPISODES, render=False):
    ...
    """
    game_state = bipedalWalker.start()
    final_fitness = 0
    ...
    for episode in range(num_episodes):
        ...
        game_state, reward, done= bipedalWalker.step(action) 
        ...
        for g in generation_list:
            for c in crossover_list:
                for m in mutation_list:
                    results_path = os.path.join(PATH_RESULTS_GENETIC_WALKER,f'POPULATION={p}_MAX-GEN={g}_CROS_RATE={c}_MUT-RATE_{m}')  
                    old_population = [BipedalWalkerIndividual() for _ in range(p)]
